...
class ApiController extends Controller {
    ...
    public function ins_featuredVideo(Request $request) {
        ...
        $VideoURL = "http://videorequestline.com/video/" . date('U') . $filename;
    ...
    public function show_testimonial(Request $request) {
        ...
        //$testimonial_data = DB::table('testimonials')->where('to_profile_id', $request->profileId)->select("*")->get();
    ...
    public function unlogin_user_RequestVideo(Request $request) {
        ...
        if (!is_null($user_favorites)) {
            ...
            //$timestamp = date('m-d-Y',strtotime(Carbon::now()->format('m-d-Y')));
            //$newTime = strtotime('%s +%d days'. $timestamp, $artist->timestamp);
    ...
    public function RequestVideo(Request $request) {
        ...
        $Requestvideo->RequestStatus = $request->RequestStatus;

        $Requestvideo->DateInterval = $request->DateInterval;

        $Requestvideo->Description = $request->Description;

        $Requestvideo->complitionDate = $request->RequestDate;

        $Requestvideo->ReqVideoPrice = $request->VideoPrice;
        ...
        if ($Requestvideo->save()) {
            ...
            $confirmation_code['name'] = $request->Name;
            ...
            $confirmation_code['current_status'] = $request->RequestStatus;

            $confirmation_code['video_delivery_time'] = $request->DateInterval;
            ...
            Mail::send('emails.api_user_requestNewVideo', $confirmation_code, function ($message) use ($request) {
            ...
            Mail::send('emails.api_artist_requestNewVideo', $confirmation_code, function ($message) use ($request) {

                $artist = Profile::where('ProfileId', $request->artist)->first();

                $message->from('noreply@videorequestline.com', 'Request for Video Request');

                $message->to('codingbrains6@gmail.com', $request->email);

                $message->bcc(config('constants.BCC_MAIL_CONFIG.admin_email'), config('constants.BCC_MAIL_CONFIG.admin_email_display_name'));

                $message->subject('A User Requested New video');
            });

            /* $response = array('return_message' => "Request Submitted succesfully");

              $passphrase = '12345';

              $artist_detail=DB::table('users')->where('profile_id','=',$artist->ProfileId)->first();
              ...
              'alert' => 'User has requested you for video',
            ...
            $response = array('return_message' => "Request Submitted successfully");

            return json_encode($response);
        } else {

            $response = array('return_message' => "Request Not Submitted successfully");
    ...
    public function UpdateProfile(Request $request) {
        ...
        if ($updateType == "Picture") {
            ...
            if ($request->type == "User") {
                ...
                $target = "images" . '/' . 'User' . '/' . str_replace(" ", "", $_POST['randomFilename']);
            } else {
                ...
                $target = "images" . '/' . 'Artist' . '/' . str_replace(" ", "", $_POST['randomFilename']);
            }
            ...
            if (move_uploaded_file($_FILES['media']['tmp_name'], $target)) {
                ...
                if (Profile::where('ProfileId', $request->profileId)->update(array('profile_path' => $profilePath))) {
        ...
        if ($updateType == "Banner") {
            ...
            if ($request->type == "User") {
                ...
                $target = "banner" . '/' . 'User' . '/' . str_replace(" ", "", $_POST['randomFilename']);
            } else {
                ...
                $target = "banner" . '/' . 'Artist' . '/' . str_replace(" ", "", $_POST['randomFilename']);
            }
            ...
            if (move_uploaded_file($_FILES['media']['tmp_name'], $target)) {
                ...
                if (Profile::where('ProfileId', $request->profileId)->update(array('BannerImg' => $profilePath))) {
        ...
        if ($validator->fails()) {
            ...
            if (Auth::attempt($user)) {
                ...
                if ($device_token == $request->device_token) {
                    ...
                    DB::table('users')->where('user_id', $user_id)->update(array('device_token' => $device_token));
                } else {



                    DB::table('users')->where('user_id', $user_id)->update(array('device_token' => $request->device_token));
                }
                ...
                if ($users->profile_path == "") {
                    ...
                    $prof_img = "http://videorequestline.com/images/Artist/default.jpg";
                } else {
