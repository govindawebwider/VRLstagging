		...
		try {
			...
			//if($charge['status']=="succeeded"){
				...
				if(count($is_found_user)==1){
					...
					$payment->profile_id = 1057;
					$payment->video_status = 'Pending';
					$payment->video_request_id = 1110;
					$payment->stripeTokenType = 'card';
					$payment->stripeToken = '23423dfgdfgdfdfg';
					...
					if($payment->save()){
						$response = array('return_message' => "if Payment succesfully");
					}
					...
					$payment->payment_type = 'Purchase';
					if($payment->save()){
						$response = array('return_message' => "else Payment succesfully");
					}
