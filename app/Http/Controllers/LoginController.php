...
//dd($request_details);
    ...
    public function view_review() {
        if (Auth::check()) {
            if (Auth::user()->type == "Admin" && session('current_type') == "Admin") {
                ...
                $update=Testimonial::where('to_profile_id', Auth::user()->profile_id)->update(['readtestimonial' => 1]);
    ...
    public function media(Request $request) {
        if (DB::table('profiles')->
                        ...
                        update(array('text_color' => $request->text_color, 'name_heading_size' => $request->name_heading_size, 'custom_css' => $request->custom_css))) {
    ...
    public function forget_password_verify(Request $request) {
        ...
        if (Auth::check()) {
            ...
            if (!is_null($result)) {
                ...
                Session::flush();
                return redirect('login')->with('login_errore', "Invalid Access!");
    ...
    public function post_password_reset(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if (DB::table('users')->where('email', $result->email)->update(['password' => $enc_pass])) {
                ...
                Session::flush();
                return redirect('login')->with('success3', "You have successfully reset your Password");
            } else {
                return redirect()->back();
            }
        }
    ...
    public function forgetpass(Request $request) {
        ...
        if ($validator->fails()) {
            ...
            if (!is_null($email)) {
                ...
                return redirect('login')->with('success', "Please Check Your Email to get Password")->with('success2', "Please Check Your Email to get Password");
            } else {
        ...
        if ($validator->fails()) {
            ...
            // } else {
                ...
                if (!empty($email)) {
                    ...
                    $diff = abs(strtotime($currentDate) - strtotime($dob));
                    $years = floor($diff / (365*60*60*24));
                    if ((int)$years < 18) {
                        return redirect('/')->with('register_error', "Age must be 18 or above 18 years");
                    }
                    else
                    {
                        if ((int)$years > 90) {
                        return redirect('/')->with('register_error', "Age could be 90 or before years");
                        }
                        else
                        {
        ...
        if ($is_found_user != null) {
            try {
                ...
                if ($charge['status'] == "succeeded") {
                    ...
                    $Requestvideo->artist_percent = ($request->artist_vidoe_price*$share->artist_share)/100;
                    $Requestvideo->system_percent = ($request->artist_vidoe_price*$share->admin_share)/100;
                    ...
                    if ($Requestvideo->save()) {
                        ...
                        $payment->artist_share = ($request->artist_vidoe_price*$share->artist_share)/100;
                        $payment->system_share = ($request->artist_vidoe_price*$share->admin_share)/100;
...
//                        });
  ...
  /*  public function verify_email($auth_pass) {
            ...
            $result = DB::table('users')->where('auth_pass','=',$auth_pass)->first();
            ...
            {   
                 ...
                 {
                     Session::put('success', 'Email already Verified ! Now Login');
                     return redirect('login');
                 }
    ...
    public function verify_email($auth_pass) {
        ...
        if (Auth::check()) {
            ...
            if (count($result) > 0) {
                if (User::where('auth_pass', '=', $auth_pass)->update(array('is_email_active' => 1, 'is_account_active' => 1, 'auth_pass' => ''))) {
                    Session::flush();
                    return redirect('login')->with('sucmsg', "Thanks for verifying your email.");
                   ...
                   Session::flush();
                    return redirect('login')->with('login_errore', "Oops..! Something Went wrong");
    ...
    public function get_dashboard() {
        ...
        if (Auth::check()) {
            ...
            if (Auth::user()->type == "Admin" && session('current_type') == "Admin") {
                ...
                $review_notification = DB::table('testimonials')->where('to_profile_id','=', Auth::user()->profile_id)
                                ->where('readtestimonial', '=', '0')->orderBy('id')->get();
                ...
                $artist_data['review_notification'] = $review_notification;
                //$artistData['profileData'] = $profileData;
                // dd($artist_data);
        ...
        if ($validator->fails()) {
            ...
            if ((int)$years < 18) {
                return redirect('ProfileUpdate')->with('error', "Age must be 18 or above 18 years");
                        ...
                        if ((int)$years > 90) {
                            return redirect('ProfileUpdate')->with('error', "Age could be 90 or before years");
                        }
                        else
                        {
